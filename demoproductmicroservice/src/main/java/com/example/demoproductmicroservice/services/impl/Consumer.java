...
import com.example.demoproductmicroservice.entity.ProductDetails;
...
import com.example.demoproductmicroservice.services.IConsumerService;
...
public class Consumer implements IConsumerService {
    ...
    @KafkaListener(topics = "updateByOrderAfterOrder", groupId = "group_product")
    ...
    @KafkaListener(topics = "updateByMerchantAfterUpdate", groupId = "group_product")
    public void listener1(String updateProductsMerch) throws IOException {
        ...
        updateProduct.setProductId(productMongoService.findByProductName(updateProductsMerch1.getProductName()).get(0).getProductId());
        productMongoService.updateQuantityCostByMerchant(updateProduct);
        System.out.println("heeeee");
        logger.info("!!!!!!!!!!!!!!!!!");
    }

    @KafkaListener(topics = "AddProduct1", groupId = "group_product")
    public void listener2(String merchant) throws IOException {
        ...
        if(productMongoService.getProduct(merchant1.getProductDetails().get(0).getProductId()) != null) {
            Product product1 = productMongoService.getProduct(merchant1.getProductDetails().get(0).getProductId());
        ...
        productMongoService.save(product);
        logger.info("!!!!!!!!!!!!!!!!!");
    }
}
